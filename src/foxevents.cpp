...
namespace foxevents {
    ...
    FoxEventQueue::FoxEventQueue(int threadCount) {
        this->events = std::vector<FoxEvent *>();
        this->threadPool = std::make_unique<foxworkers::FoxHole>(threadCount);
    }
    ...
    void FoxEventQueue::runInBackGround(FoxEvent *event) {
        this->threadPool->add(event->event, event->args);
    }

    bool FoxEventQueue::isThreadPoolEmpty() {
        return this->threadPool->isDone();
    }
    void FoxEventQueue::endThreadPool() {
